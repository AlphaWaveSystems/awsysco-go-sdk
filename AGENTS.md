...
| Field | Value |
|---|---|
| Agent ID | `awsysco-go-sdk` |
| Trust level | `worker` |
| Model tier | `coder` → `claude-sonnet-4-6` |
...
identity is `harness-ctl mcp-bridge awsysco-go-sdk`, registered as a stdio MCP server:

```json
{
  "mcpServers": {
    "harness-awsysco-go-sdk": {
      "type": "stdio",
      "command": "harness-ctl",
      "args": ["mcp-bridge", "awsysco-go-sdk"]
    }
  }
}
```
